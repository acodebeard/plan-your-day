...
namespace {
	...
	if ( ! function_exists( 'current_user_can' ) ) {
		function current_user_can( string $capability ): bool {
			return 'manage_options' === $capability && ! empty( $GLOBALS['plan_your_day_test_can_manage_options'] );
		}
	}
...
namespace Acodebeard\PlanYourDay\Tests {
	...
	final class PlannerBlockTest extends TestCase {
		protected function setUp(): void {
			...
			$GLOBALS['plan_your_day_test_can_manage_options']  = false;
		...
		public function test_register_uses_block_metadata_with_render_callback(): void {
			...
			self::assertCount( 2, $GLOBALS['plan_your_day_test_registered_blocks'] );
			self::assertSame(
				'metadata',
				$GLOBALS['plan_your_day_test_registered_blocks'][0]['type']
			);
			self::assertSame(
			...
			self::assertSame(
				'named',
				$GLOBALS['plan_your_day_test_registered_blocks'][1]['type']
			);
			self::assertSame(
				'plan-your-day/planner',
				$GLOBALS['plan_your_day_test_registered_blocks'][1]['block_type']
			);
			self::assertSame(
				[ $block, 'render' ],
				$GLOBALS['plan_your_day_test_registered_blocks'][1]['args']['render_callback'] ?? null
			);
		}
		...
		public function test_render_enqueues_frontend_assets_and_uses_shared_renderer_output(): void {
			...
			self::assertStringContainsString( '"bootstrapUrl":"https:\/\/example.test\/wp-json\/waypoints\/v1\/bootstrap"', $output );
			...
			self::assertStringNotContainsString( 'data-plan-api-call-counter', $output );
		}

		public function test_render_shows_api_call_counter_only_for_admin_when_debug_option_is_enabled(): void {
			$GLOBALS['plan_your_day_test_options'][ Settings::OPTION_NAME ] = Settings::sanitize(
				array_merge(
					Settings::defaults(),
					[
			...
			$GLOBALS['plan_your_day_test_can_manage_options'] = false;
			$visitor_output = $block->render( [ 'actionUrl' => 'https://example.test/planner' ] );

			$GLOBALS['plan_your_day_test_can_manage_options'] = true;
			$admin_output = $block->render( [ 'actionUrl' => 'https://example.test/planner' ] );

			self::assertStringNotContainsString( 'data-plan-api-call-counter', $visitor_output );
			self::assertStringContainsString( 'data-plan-api-call-counter', $admin_output );
			self::assertStringContainsString( 'API calls:', $admin_output );
		}
