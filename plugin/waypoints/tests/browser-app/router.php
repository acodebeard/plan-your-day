...
switch ( $request_path ) {
	...
	case '/wp-json/waypoints/v1/bootstrap':
		plan_your_day_browser_dispatch_rest( 'bootstrap' );
		return;

	case '/wp-json/waypoints/v1/browse':
		plan_your_day_browser_dispatch_rest( 'browse' );
		return;

	case '/wp-json/waypoints/v1/route':
...
function plan_your_day_browser_render_page( string $page ): void {
	...
	$title    = 'Waypoints: Trip Planner Browser Smoke';
	$content  = '<main class="browser-app"><h1>Waypoints: Trip Planner Browser Smoke</h1><p>No planner is rendered on this page.</p></main>';

	if ( 'shortcode' === $page ) {
		$title   = 'Waypoints: Trip Planner Shortcode Smoke';
		...
		$title   = 'Waypoints: Trip Planner Narrow Shortcode Smoke';
		$content = sprintf(
		...
		$title   = 'Waypoints: Trip Planner Block Smoke';
...
function plan_your_day_browser_dispatch_rest( string $route_name ): void {
	...
	$request = new WP_REST_Request( 'POST', '/waypoints/v1/' . $route_name );
