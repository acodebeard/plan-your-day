...
namespace Acodebeard\PlanYourDay\Tests {
	...
	final class SettingsPageTest extends TestCase {
		...
		public function test_register_adds_admin_only_api_call_counter_debug_toggle(): void {
			$settings_page = $this->settings_page();

			$settings_page->register();

			$field = $GLOBALS['plan_your_day_test_settings_fields']['plan_your_day_debug_api_counter_enabled'] ?? [];

			self::assertSame( 'API call counter', $field['title'] ?? null );
			self::assertSame( 'checkbox', $field['args']['type'] ?? null );
		...
		public function test_enqueue_assets_only_runs_on_the_settings_screen(): void {
			...
			self::assertSame(
				...
				$GLOBALS['plan_your_day_test_enqueued_scripts']['waypoints-admin-settings']['deps']
			);
		}
