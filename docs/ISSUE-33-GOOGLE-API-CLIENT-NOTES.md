...
Caching should be added as a wrapper/decorator around `GoogleApiClientInterface` in issue #34. Cache keys must be derived from request inputs and a hash of the server-side key, never from the raw API key.
